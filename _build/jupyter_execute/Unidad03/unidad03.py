...
1.  Se realiza el *diagrama de flujo del sistema*: En ese diagrama se
    ...
    El nivel del depósito depende de los caudales de entrada y salida
    del mismo. \`\`\`
...
2.  Planteamiento del *modelo matemático*: Un modelo matemático es un
    ...
    En el momento de plantear el modelo matemático hay que explicitar
    todas las hipótesis o simplificaciones realizadas. En este caso se
    ...
    Teniendo en cuenta las hipótesis anteriores la ec. {eq}`BMM` queda
    como:
...
3.  Definición de las *variables de desviación*: Debido a que lo
    ...
    Restando ambos balances macroscópicos de materia (ecs. {eq}`BMM` y
    {eq}`BMMe`):
...
(transformada)= La transformada de Laplace como herramienta útil
————————————————
...
`{admonition} Ejemplo La aplicación de la transformada de Laplace a una función es sencilla disponiendo de las tablas de transformadas de Laplace y del conocimiento de las propiedades anteriores. En el Problema 2.1 se muestra cómo se aplica.`
...
`{admonition} Ejemplo El problema 3.6 muestra un ejemplo de sistema en serie. Se trata de dos depósitos de almacenamiento en serie. La curiosidad es que se plantea el sistema con interacción (el mostrado en la figura [\[fig:serie\]](#fig:serie){reference-type="ref" reference="fig:serie"}) y el sistema de tanques sin interacción, para el que no es aplicable de manera directa la ecuación [\[ec:serie\]](#ec:serie){reference-type="ref" reference="ec:serie"}.`
...
<a href="#translacion" data-reference-type="ref" data-reference="translacion">[translacion]</a>
(ecuación
<a href="#ec:translacion" data-reference-type="ref" data-reference="ec:translacion">[ec:translacion]</a>),
...
`{admonition} Ejemplo En el Problema 3.4 se puede comprobar las diferencias y similitudes en la respuesta de un proceso a una entrada en escalón y en impulso.`
...
<a href="#sec:transformadas" data-reference-type="ref" data-reference="sec:transformadas">1.7</a>):
...
donde $\tau = R A$ es la constante de tiempo y tiene dimensiones de
tiempo.

Cuanto mayor es $\tau$ más lenta es la respuesta, más tarda el sistema
en alcanzar el estado estacionario. Se comprueba que cuanto menor es la
sección del tanque más rápida es la respuesta. Si $\tau$ es grande se
dice que el sistema presenta una gran inercia.
...
<a href="#ec:dos%20raices" data-reference-type="ref" data-reference="ec:dos raices">[ec:dos raices]</a>
...
Para encontrar el numerador *C* de la ecuación
<a href="#ec:tres%20raices" data-reference-type="ref" data-reference="ec:tres raices">[ec:tres raices]</a>
se debe tomar la segunda derivada. Generalizando al término $A_j$ de una
raíz de orden *N* en $p_1$:
...
(transformadas)= Transformadas de Laplace ————————
